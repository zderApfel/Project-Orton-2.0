...
func _on_gui_input(event: InputEvent) -> void:
	if event.is_action_released("primary_action") or event.is_action_released("controller_ui_accept"):
		print(item.Item_Name)
	elif event.is_action_released("secondary_action") or event.is_action_released("controller_ui_cancel"):
		Helpers.drop_item(item, self)
